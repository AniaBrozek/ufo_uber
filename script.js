...
async function fetchUsers() {
  try {
    const response = await fetch('http://lightcoral-badger-606203.hostingersite.com:443/users');
...
async function fetchOffers() {
  try {
    const response = await fetch('http://lightcoral-badger-606203.hostingersite.com:443/offers');
    const data = await response.json();
    objOfert.push(...data);
...
async function updateOffer(price, driver_username, if_accepted, offerId) {
  try {
    const response = await fetch(`http://lightcoral-badger-606203.hostingersite.com:443/offers/${offerId}`, {
      method: 'PUT',
      headers: {
        'Content-Type': 'application/json',
      },
